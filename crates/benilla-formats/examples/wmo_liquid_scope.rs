//! Where WMO liquid scoping can claim the wrong position, read off every placed WMO (from an ADT
//! or a WDT), each hit with a `.go`. `cargo run -p benilla-formats --example wmo_liquid_scope`
//!
//! - STOREY: a pool's wet cells over another group's floor, submerged by placement scoping alone.
//! - OVERHANG: how far wet cells reach past their own group's XY box, up to 25 yd, which is why
//!   benilla bounds a pool by that box's Z floor only where the reference tests the whole box
//!   (`0x6a4e00`).
//! - DRY-GROUP: a group with a `groupLiquid` other than `0xf` and no MLIQ, which the reference
//!   answers with an unconditional hit at height `FLT_MAX` (`0x6b9f10`).
//!
...
/// MOGP `groupLiquid` (`+0x34`) for a group holding no liquid.
const NO_LIQUID: u32 = 0xf;

/// Half the 34133⅓-yard map (32 tiles × 533⅓), the MODF-to-world origin shift.
const MAP_CENTER: f32 = 17066.666;

struct Group {
    liquid_type: u32,
    liquid: Option<LiquidMesh>,
    /// MOGP bounding box in model space, at `+0x0c`.
    lo: [f32; 3],
    hi: [f32; 3],
}
...
/// Whether the group carries an MLIQ sub-chunk after MOGP's 68-byte header; [`Group::liquid`] is
/// also `None` for an all-holes grid.
fn has_mliq(bytes: &[u8]) -> bool {
    chunk(bytes, b"PGOM").is_some_and(|m| m.len() > 0x44 && chunk(&m[0x44..], b"QILM").is_some())
}

/// A MODF placement's model-to-world rotation in the WoW frame (Z up), the `in_wow` half of
/// `benilla_assets::coords::placement_rotation`: `Rx(90°)·Ry(ry-180°)·Rz(-rx)·Rx(rz-90°)`.
...
fn main() -> anyhow::Result<()> {
    ...
    // A WMO-only map has no ADT: its world is the WDT's single `MODF` (20 of the 43 shipped maps).
    ...
    for name in &wdts {
        ...
        let lower = name.to_lowercase();
        let map = lower.split(['\\', '/']).nth(2).unwrap_or("?").to_string();
        // No `MAP_CENTER - v` remap: a global WMO's MODF is in world coords already, and every
        // shipped one sits at (0,0,0).
        placed
    ...
    // groupLiquid value -> (groups with an MLIQ chunk, groups without).
    ...
    for (model, sites) in &placed {
        ...
        for gi in 0.. {
            ...
            if has_mliq(&gb) {
                ...
                if g.liquid_type != NO_LIQUID {
                    // Aim at the room's box centre, just off its floor, not the placement origin.
        ...
        // STOREY: for each pool, the lower groups its wet cells hang over.
        for (ai, a) in groups.iter().enumerate() {
            ...
            for (bi, b) in groups.iter().enumerate() {
                ...
                // Where the query fires: under the pool's height, in a wet cell, on B's floor.
                ...
                for (xy, h) in &cells {
                    ...
                    // Skip B when its box holds the liquid height: that submersion is real.
                ...
                if let Some((xy, h)) = best {
                    ...
                    // Whether the floor rule (nothing below the pool's group box) closes this.
    ...
    // `groupLiquid` over every group file in the archive, placed or not.
